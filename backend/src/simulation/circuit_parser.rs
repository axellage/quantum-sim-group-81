...
fn parse_gate_hashmap(gate_string: &str) -> QuantumGate {
    // Define the hashmap mapping gate strings to QuantumGate instances
    let mut gate_map: HashMap<&str, QuantumGate> = HashMap::new();
    
    // Insert mappings for single-qubit gates
    gate_map.insert("I", QuantumGate::i_gate());
    gate_map.insert("H", QuantumGate::h_gate());
    gate_map.insert("X", QuantumGate::x_gate());
    gate_map.insert("Y", QuantumGate::y_gate());
    gate_map.insert("Z", QuantumGate::z_gate());
    gate_map.insert("T", QuantumGate::t_gate());
    gate_map.insert("S", QuantumGate::s_gate());
    gate_map.insert("CZ", QuantumGate::cz_gate());
    
    // Define mappings for other gates
    let mut other_gates = HashMap::new();
    other_gates.insert("SWAP-1", QuantumGate::swap_gate());
    other_gates.insert("CCNOT-1", QuantumGate::ccnot_gate());
    other_gates.insert("CNOT-1", QuantumGate::cnot_gate());
    other_gates.insert("CNOT-2", QuantumGate {
        matrix: arr2(&[[Complex::new(1.0_f64, 0.0_f64)]]),
        size: 0,
    });
    other_gates.insert("CCNOT-2", QuantumGate {
        matrix: arr2(&[[Complex::new(1.0_f64, 0.0_f64)]]),
        size: 0,
    });
    other_gates.insert("CCNOT-3", QuantumGate {
        matrix: arr2(&[[Complex::new(1.0_f64, 0.0_f64)]]),
        size: 0,
    });
    other_gates.insert("SWAP-2", QuantumGate {
        matrix: arr2(&[[Complex::new(1.0_f64, 0.0_f64)]]),
        size: 0,
    });
    
    // Merge all gate mappings
    gate_map.extend(other_gates);

    // Retrieve the QuantumGate instance from the hashmap
    gate_map.get(gate_string).cloned().unwrap_or_else(|| panic!("Invalid gate"))
}
