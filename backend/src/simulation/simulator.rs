...
impl UnparsedCircuit {
    pub fn new(input: Vec<Vec<String>>) -> UnparsedCircuit {
        UnparsedCircuit {circuit = input}
    }
}
...
fn simulate(circuit_to_simulate: ParsedCircuit) -> CircuitStates {
    ...
    for (index, gates_at_step) in circuit_to_simulate.circuit.into_iter().enumerate() {
        let state_at_step: TimeStep = step(state_list.clone().states[index - 1], gates_at_step);
...
fn initialize_qubit_states(amount_of_qubits: i32) -> CircuitStates {
    let mut initial_states: Vec<TimeStep> = vec![];
    for i in 0..amount_of_qubits {
        let step: TimeStep = TimeStep {
...
fn step(states_before_gate: TimeStep, gates_at_step: GatesAtStep) -> TimeStep {
    let states_at_step: TimeStep = process_gates(gates_at_step, states_before_gate);
    states_at_step
}

fn process_gates(gates_at_step: GatesAtStep, states_before_gate: TimeStep) -> TimeStep {
    gates_at_step.into_iter().map(|gate_with_operands| use_gate(gate_with_operands, states_before_gate)).collect();
}

fn use_gate(gate_with_operands: GateWithOperands, states_before_gate: TimeStep) -> QuantumState {
...
fn combine_states_into_gate_if_needed(qubits_in_gate: Vec<i32>, states_before_gate: TimeStep) -> QuantumState {
    let state_into_gate: 
